...
## 2026-06-30 - Accessible Combobox Pattern for Autocomplete
**Learning:** For custom autocomplete inputs (like the address search in this app), simple `keydown` handlers are not enough for screen readers. The `role="combobox"` pattern is required. Toggling `aria-expanded` and syncing `aria-activedescendant` on the input with dynamically generated `role="option"` IDs ensures screen readers announce the currently focused suggestion correctly during keyboard navigation.
**Action:** Always implement the `role="combobox"` pattern for autocomplete inputs. Ensure `aria-expanded` is toggled dynamically, link the dropdown container via `aria-controls`, use `role="listbox"` and `role="option"` for the suggestions, and manage focus dynamically using `aria-activedescendant` linked to the active suggestion's ID.
