...
*The Forecast page, showing sunrise and sunset quality for each saved location.*
...
Pull requests to `main` (and reusable `workflow_call`) run lint, audit, tests, coverage, and Wrangler dry-run with no production secrets (`validate.yml`). Pushes to `main` (or manual dispatch) run the full production pipeline: validate → prepare → deploy Worker → deploy Pages → verify → release (`production.yml`). Exact-identifier rollback is a separate manual workflow (`rollback.yml`) that restores a prior Worker version and/or Pages deployment.
...
Delivery can be email-only, Pushover-only, both, or disabled. Channel preferences live in D1; provider transport credentials live in Cloudflare Secrets Store and are managed through the Notifications UI. Delivery requires the corresponding Secrets Store document to be marked `configured: true`; there is no legacy Worker-secret fallback.

Each report snapshot creates one D1 outbox job per enabled channel, then attempts delivery immediately. Jobs use at-least-once semantics: the dispatcher leases a job for one minute and retries transient failures after 1 minute, 5 minutes, 30 minutes, and 2 hours (five attempts total). A later hourly cron also processes due jobs.

Use the tab's test buttons and delivery history to test or retry a failed channel. Tests use the same outbox/dispatcher path as reports. Pushover titles are limited to 250 characters, messages to 1,024, and dashboard URLs to 512. No emergency priority is supported.
...
Access is initialization-only. Run these locally with instance configuration in `.env`; they are not part of the production pipeline. One-time setup context: [docs/cloudflare-credentials.md](docs/cloudflare-credentials.md).
